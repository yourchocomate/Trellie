...
const ManageTeam = ({projectId}) => {
    const [people, setPeople] = useState({team: [], suggestion: []});

    const auth = useAuth();
    const {formState, onChange, resetForm} = useStateForm({...initialFormState, project_id: projectId});
    
    const { addTeamMember, getTeamSuggestion } = useProject(formState, (res) => {
        resetForm();
        auth.setNotify({ show: true, type: 'success', message: res });
    },(err) => {
    ...
    useEffect(() => {
        getTeamSuggestion(projectId).then(res => {
            if(res) setPeople(res.data);
        });
    },[auth.notify]);
    ...
    return(
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 my-4">
            <div className="card py-4">
                <div className="max-w-md mx-auto sm:max-w-3xl py-8">
                    <div>
                        ...
                        <form className="mt-6 sm:flex sm:items-center" onSubmit={addTeamMember}>
                    ...
                    <div className="mt-10">
                        ...
                        {people.suggestion.map((person, personIdx) => (
                            <li key={personIdx}>
                            <button
                                onClick={(e) => handleButtonClick(e, person.email)}
                                type="button"
                    ...
                    <div className="mt-10">
                        ...
                        {people.team.map((person, personIdx) => (
